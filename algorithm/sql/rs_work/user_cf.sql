...
create table TMP_USER_STATISTICS as
select
	t.user_id,
	t.item_id,
	t.value,
	sum(t.value*t.value) over(partition by t.user_id) as sum_square
from
	interest_rate t
;
...
(
	...
	(
		...
		from
			TMP_USER_STATISTICS t1
		join
			TMP_USER_STATISTICS t2
		on
	...
	group by
		user1, sum_square1, user2, sum_square2
) t
;
